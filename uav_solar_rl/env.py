...
ACTIONS = [
    ...
    "ascend", "descend", "hover", "charge", "inspect",
]
...
class UAVSolarEnv:
    ...
    def initial_state(self) -> State:
        x, y, z = self.base
        return (x, y, z, self.max_battery, self.scenario["initial_wind"], "Short", "Short", 0)
    ...
    def sample_from_dist(self, dist: Dict[str, float]) -> str:
        ...
        for k, p in dist.items():
            ...
            if r <= acc:
                return k
        return last  # numerical fallback
        ...
        for nw, pw in self.wind_transition[wind].items():
            for nq1, pq1 in self.queue_transition[q1].items():
                for nq2, pq2 in self.queue_transition[q2].items():
                    yield nw, nq1, nq2, float(pw) * float(pq1) * float(pq2)

    def transition_distribution(self, state: State, action: str) -> List[Tuple[float, State, float, bool, Dict[str, object]]]:
        """Exact one-step transition distribution for value iteration."""
        outcomes = []
        for nw, nq1, nq2, p_exo in self.possible_next_exogenous(state[4], state[5], state[6]):
            ns, reward, done, info = self._deterministic_part(state, action, nw, nq1, nq2)
            outcomes.append((p_exo, ns, reward, done, info))
        return outcomes

    def step(self, action: str) -> StepResult:
        wind, q1, q2 = self.state[4], self.state[5], self.state[6]
        ...
        ns, reward, done, info = self._deterministic_part(self.state, action, nw, nq1, nq2)
        self.steps += 1
    ...
    def _deterministic_part(self, state: State, action: str, next_wind: str, next_q1: str, next_q2: str):
        ...
        if action in ["move_N", "move_S", "move_E", "move_W", "ascend", "descend"]:
            ...
            if not self.in_bounds(next_pos):
                ...
                # Wind may drift the UAV after horizontal/vertical movement.
                drifted = self.wind_drift_position(next_pos, wind)
                if self.in_bounds(drifted):
                    next_pos = drifted
                    if drifted != self.move_position(pos, action):
            ...
            if pos in self.charging_pads:
                ...
                reward += float(self.R["charge_short_queue_cost"] if queue == "Short" else self.R["charge_long_queue_cost"])
        ...
        nx, ny, nz = next_pos
        next_state: State = (nx, ny, nz, max(0, min(self.max_battery, next_battery)), next_wind, next_q1, next_q2, next_mask)
