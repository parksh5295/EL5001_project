...
from uav_solar_rl.algorithms import evaluate_policy, q_learning, q_to_policy, rollout, sarsa, value_iteration
...
def save_metrics(rows, out_path: Path):
    ...
    fieldnames = ["algorithm", "mean_return", "std_return", "success_rate", "avg_steps", "battery_failures", "no_fly_violations", "restricted_visits", "charge_count"]
...
def print_rollout(rows):
    ...
    print("-" * 90)
    for row in rows[:30]:
        print(f"{row['t']:2d} | {row['state']} | {row['action']:8s} | {row['reward']:7.1f} | {row['info']}")
    if len(rows) > 30:
        print("... truncated ...")


def main():
    parser = argparse.ArgumentParser(description="3D UAV solar inspection MDP with Value Iteration, Q-learning, and SARSA")
    ...
    vi_policy, V = value_iteration(env, gamma=0.95, theta=1e-5, max_iter=40)
    vi_metrics = evaluate_policy(factory, lambda s: vi_policy.get(s, "hover"), episodes=args.eval_episodes)
    ...
    q_metrics = evaluate_policy(factory, lambda s: q_policy.get(s, "hover"), episodes=args.eval_episodes)
    ...
    sarsa_metrics = evaluate_policy(factory, lambda s: sarsa_policy.get(s, "hover"), episodes=args.eval_episodes)
    ...
    sample = rollout(UAVSolarEnv(scenario_path, seed=100), lambda s: q_policy.get(s, "hover"))
